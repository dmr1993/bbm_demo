...
namespace EarmarkProduceTerminalBLL
{
    ...
    public class LCS_Reader
    {
        [DllImport("LKT5103A.dll")]
        ...
        {
            f_Connected = EK_Open(1);   //LKT5103U.dll中的连接接口
            return f_Connected;
        }
        ...
        {
            int res;
            res = EK_Close();   //LKT5103U.dll中的断开连接接口
            if (0 != res)
                return res;
            else
                return 0;
        ...
        public int SendCommand(int cmdlen, byte[] cmd, byte[] res, int[] reslen)        
        {
            ...
            r = EK_Exchange_APDU(cmdlen, Marshal.UnsafeAddrOfPinnedArrayElement(cmd, 0),
                Marshal.UnsafeAddrOfPinnedArrayElement(reslen, 0), Marshal.UnsafeAddrOfPinnedArrayElement(res, 0));//LKT5103U.dll中的发送APDU接口
            if (0 != r)
                return r;
            else
                return 0;
        ...
        {
            int res;
            res = EK_Reset(out AtrL, Marshal.UnsafeAddrOfPinnedArrayElement(atr, 0));   //LKT5103U.dll中的复位接口
            if (0 != res)
                return res;
            else
            return AtrL;
